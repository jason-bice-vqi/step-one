...
public class AuthModule : IServiceModule
{
    public void Configure(IConfiguration configuration, IServiceCollection services, IWebHostEnvironment env)
    {
        ...
        services.AddAuthentication()
            .AddJwtBearer(
                Schemes.InitialAzureAdJwtAuthScheme,
                options =>
                {
                    ...
                    options.Events = new()
                    {
                        ...
                        {
                            Log.Error(
                                "Azure AD token validation FAILED for scheme {Scheme}: {Message} {Stack}",
                                v.Scheme,
                                v.Exception.Message,
            ...
            .AddJwtBearer(
                ...
                options =>
                {
                    ...
                    options.Events = new()
                    {
                        ...
                        {
                            Log.Error(
                                "Native JWT validation FAILED for scheme {Scheme}: {Message} {Stack}",
                                v.Scheme,
                                v.Exception.Message,
                                v.Exception.StackTrace);
